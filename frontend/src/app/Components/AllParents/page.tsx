...
import { deleteParent, fetchParents } from '@/config/utils';
              ...
              <table className='w-full border-collapse'>
                ...
                <tbody>
                  ...
                  .filter((parent) => {
                    ...
                    const username = parent.user.username.toLowerCase();
                  ...
                  .map((parent, index) => (
                    <tr key={index} className='hover:bg-gray-50'>
                      ...
                      <td className='border border-gray-500 p-2'>{parent.user?.first_name}</td>
                      <td className='border border-gray-500 p-2'>{parent.user?.last_name}</td>
                      <td className='border border-gray-500 p-2'>{parent.user?.username}</td>
                      <td className='border border-gray-500 p-2'>{parent.user?.email}</td>
                      <td className='border border-gray-500 p-2'>{parent.phone}</td>
                        ...
                        {parent.profile_picture ? (
                          <div className="p-1 border border-gray-300 rounded-lg inline-block shadow-sm">
                            <Image
                              ...
                              alt={`${parent.user.first_name} profile`}
                              width={40}
                              height={40}
                              unoptimized
                      ...
                      <td className='border border-gray-500 p-2'>{parent.address}</td>
                      <td className='border border-gray-500 p-2'>{parent.parent_code}</td>
                      <td className='border border-gray-500 p-2'>
                        {/* UPDATE AND DELETE */}
                        <div className="flex items-center justify-center gap-3">
                          <Link href={`/Components/parentForm?id=${parent.id}`}>
