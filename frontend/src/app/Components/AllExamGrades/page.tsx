...
import { deleteExam, deleteExamGrade, fetchExamGrades, fetchExams } from '@/config/utils';
...
const AllExamGradesPage = () => {
  ...
  const fetchExamGrade = async () => {
    const data = await fetchExamGrades();
    if (data.error) {
      setError(data.error);
      return;
    }
  ...
  const handleDelete = async (id: string) => {
    ...
    const confirmDelete = window.confirm("Delete ExamGrade?");
    ...
    if (confirmDelete) {
      ...
      alert("ExamGrade Deleted Successfully");
      fetchExamGrade()
    }
  }

  // filter by name, exam code, subject and date, and term
  const filteredExamGrades = examGrades.filter((examGrade) => examGrade.exam_name.toLowerCase().includes(searchQuery.toLowerCase())
   || examGrade.student.toLowerCase().includes(searchQuery.toLowerCase()) || examGrade.subject.toLowerCase().includes(searchQuery.toLowerCase()))
          ...
          <div className="p-4">
            ...
            <div className="overflow-x-auto">
              <table className='w-full border-collapse'>
                ...
                <tbody>
                  {filteredExamGrades.map((examGrade, index) => (
                    <tr key={examGrade.id}>
                      <td className='border border-gray-500 p-2'>{index + 1}</td>
                      <td className='border border-gray-500 p-2'>{examGrade.exam_name}</td>
                      <td className='border border-gray-500 p-2'>{examGrade.student}</td>
                      <td className='border border-gray-500 p-2'>{examGrade.subject}</td>
                      <td className='border border-gray-500 p-2'>{examGrade.marks}</td>
                      <td className='border border-gray-500 p-2'>{examGrade.grade}</td>
                      <td className='border border-gray-500 p-2'>
                        {/* UPDATE AND DELETE */}
                        <div className="flex items-center justify-center gap-3">
                          <Link href={`/Components/examGradeForm?id=${examGrade.id}`}>
                            <Image src='/update.png' alt='' width={20} height={20} className='rounded-md bg-blue-400 w-8 h-8 p-2' />
                          </Link>
                          <button onClick={() => handleDelete(examGrade.id)}>
...
export default AllExamGradesPage